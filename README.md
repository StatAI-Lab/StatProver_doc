...
> *Define the following partitioned sets:* $\Psi_D = (\Psi'_B \cap \Psi'_C) \setminus \Psi'_A$ *,* $\Psi_b = \Psi'_B \setminus (\Psi'_A \cup \Psi'_C)$ *, and* $\Psi_c = \Psi'_C \setminus (\Psi'_A \cup \Psi'_B)$ *.*
...
>
> *(1) if* $\Psi_D \neq \varnothing$, *there exists at least one edge* $a \to b$ *where* $a \in \Psi_D$ *and* $b \in (\Psi_b \cup X_B^*) \cup (\Psi_c \cup X_C^*)$ *;*
>
> *(2) if* $\Psi_D = \varnothing$, *then* $\Psi'_B \perp \Psi'_C \mid \Psi'_A$.
...
> 2. *Establish that any node* $a \in \Psi_D$ *must lie on some directed path to an observable in* $X_B^*$ *and on some directed path to an observable in* $X_C^*$, *because* $a$ *belongs to both modules* $\Psi_B$ *and* $\Psi_C$.
> 3. *Derive, for the case* $\Psi_D \neq \varnothing$, *that along at least one such directed path starting from a node in* $\Psi_D$, *the first edge leaving the overlap must be of the form* $a \to b$ *with* $a \in \Psi_D$ *and* $b \in (\Psi_b \cup X_B^*) \cup (\Psi_c \cup X_C^*)$, *since the path must eventually reach a target observable outside the overlap.*
> 4. *Characterize the case* $\Psi_D = \varnothing$ *by showing that then* $\Psi_B \cap \Psi_C \subseteq \Psi_A$, *so every common node between the two modules is contained in the conditioning set* $\Psi_A$, *while nodes unique to one module cannot be descendants of nodes outside that module because* $ch(\Psi^{M^c}) = \varnothing$ *for Rule-1 modules.*
...
>
> 1. *Set up the decomposition of* $\Psi_B \cup \Psi_C$ *into* $\Psi_D$, $\Psi_b$, $\Psi_c$, *and* $\Psi_A$, *and record the Rule-1 facts that every node in a module has a directed path to an observable in its initialization set and that no node outside a module can be a child of a node inside that module.*
> 2. *For any* $a \in \Psi_D$, *use membership in both* $\Psi_B$ *and* $\Psi_C$ *to obtain a directed path from* $a$ *to some observable in* $X_B^*$ *and a directed path from* $a$ *to some observable in* $X_C^*$.
> 3. *Prove claim 1: along at least one such path from* $a \in \Psi_D$, *identify the first vertex reached outside* $\Psi_D$; *use the prohibition on edges from module nodes to nodes outside the module to exclude* $\Psi_S$, *and conclude that the corresponding first exit edge must be* $a' \to b$ *with* $a' \in \Psi_D$ *and* $b \in (\Psi_b \cup X_B^*) \cup (\Psi_c \cup X_C^*)$.
> 4. *Prove the structural consequence of* $\Psi_D = \varnothing$: *then* $(\Psi_B \cap \Psi_C) \subseteq \Psi_A$, *so outside* $\Psi_A$ *the modules* $\Psi_B$ *and* $\Psi_C$ *are disjoint; moreover, by the Rule-1 closure property, a directed path starting in* $\Psi_B \setminus \Psi_A$ *cannot leave* $\Psi_B$ *except possibly through nodes already shared with another module, which are absent outside* $\Psi_A$, *and symmetrically for* $\Psi_C$.
> 5. *Conclude claim 2: when* $\Psi_D = \varnothing$, *any path connecting a node of* $\Psi_B$ *to a node of* $\Psi_C$ *must intersect* $\Psi_A$; *therefore conditioning on* $\Psi_A$ *separates the two modules, yielding* $\Psi_B \perp \Psi_C \mid \Psi_A$.
...
>
> 1. *Decomposition and Module Properties: Define the disjoint sets* $\Psi_D$, $\Psi_b$, *and* $\Psi_c$ *partitioning the modules outside* $\Psi_A$. *The condition* $ch(\Psi'^{M^{c}}) = \varnothing$ *states that the set of children of the complement of* $M$ *is empty. Formally, if* $u \notin \Psi'_M$, *then* $u$ *has no children in* $G$. *This implies that all edges in* $G$ *must originate from nodes within a module.*
> 2. *Directed Paths from the Overlap: For any* $a \in \Psi_D$, *there exists a directed path to* $X_B^*$ *and a directed path to* $X_C^*$. *By the ancestral closure property derived in step 1, all ancestors of these paths are contained in their respective modules.*
> 3. *Proof of Claim 1: Suppose* $\Psi_D \neq \varnothing$ *and let* $a \in \Psi_D$. *Since* $a \notin \Psi_A$, *we examine the path to* $X_B^*$. *The path must eventually exit* $\Psi_A$ *to reach* $X_B^*$ *or a module-specific node. In all cases, there exists an edge* $a' \to b$ *where* $a' \in \Psi_D$ *and* $b$ *lands in the specific union* $(\Psi_b \cup X_B^*) \cup (\Psi_c \cup X_C^*)$ *or leads directly to it.*
> 4. *Structural Consequence of Empty Overlap: If* $\Psi_D = \varnothing$, *then* $\Psi'_B \cap \Psi'_C \subseteq \Psi'_A$. *The property* $ch(\Psi'^{M^{c}}) = \varnothing$ *implies that for any module, there are no edges from the outside. Consequently, any node in* $\Psi'_B \setminus \Psi'_A$ *can only have parents in* $\Psi'_B$.
...
>
> 1. *Decomposition and Module Properties: Define the disjoint sets partitioning the modules. The condition* $ch(\Psi'^{M^{c}}) = \varnothing$ *dictates that no node outside a module can have a child inside that module. Formally, if* $v \in \Psi'_M$ *and* $u \to v \in \mathcal{E}$, *then* $u \in \Psi'_M$. *By induction, all ancestors of nodes in* $\Psi'_M$ *are strictly contained within* $\Psi'_M$.
> 2. *Directed Paths from the Overlap: For any* $a \in \Psi_D$, *valid directed paths to* $X_B^*$ *and* $X_C^*$ *exist. Due to the corrected ancestral closure property, transitions from internal nodes to external unassigned nodes in* $\Psi'_S$ *are strictly prohibited.*
> 3. *Proof of Claim 1: We employ a proof by contradiction focused on first edge positioning. Let* $a' \to b$ *be the first edge leaving the intersection* $\Psi_D$ *along the path to* $X_B^*$. *This locks the condition* $a' \in \Psi_D$ *and* $b \notin \Psi_D$. *Since* $a' \in \Psi'_B$, *its child* $b$ *must remain in* $\Psi'_B$ *to avoid violating ancestral closure. Since* $b$ *cannot belong to* $\Psi'_S$ *or the conditioning set* $\Psi'_A$, *it must transition into the remaining valid subsets, proving* $b \in (\Psi_b \cup X_B^*) \cup (\Psi_c \cup X_C^*)$.
> 4. *Structural Consequence of Empty Overlap: If* $\Psi_D = \varnothing$, *then* $\Psi'_B \cap \Psi'_C \subseteq \Psi'_A$. *The rigorous closure property ensures that disjoint submodules cannot share edges without traversing the conditioning set* $\Psi'_A$.
