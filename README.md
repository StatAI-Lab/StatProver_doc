...
> 3. *Derive, for the case* $\Psi_{BC\!-\!} \neq \varnothing$, *that along at least one such directed path starting from a node in* $\Psi_{BC\!-\!}$, *the first edge leaving the overlap must be of the form* $a \to b$ *with* $a \in \Psi_{BC\!-\!}$ *and* $b \in (\Psi_{\mathrm{B}\!-\!} \cup X_B^*) \cup (\Psi_{\mathrm{C}\!-\!} \cup X_C^*)$, *since the path must eventually reach a target observable outside the overlap.*
...
>
> 1. *Set up the decomposition of* $\Psi_B \cup \Psi_C$ *into* $\Psi_{BC\!-\!}$, $\Psi_{\mathrm{B}\!-\!}$, $\Psi_{\mathrm{C}\!-\!}$, *and* $\Psi_A$, *and record the Rule-1 facts that every node in a module has a directed path to an observable in its initialization set and that no node outside a module can be a child of a node inside that module.*
...
> 3. *Prove claim 1: along at least one such path from* $a \in \Psi_{BC\!-\!}$, *identify the first vertex reached outside* $\Psi_{BC\!-\!}$; *use the prohibition on edges from module nodes to nodes outside the module to exclude* $\Psi_S$, *and conclude that the corresponding first exit edge must be* $a' \to b$ *with* $a' \in \Psi_{BC\!-\!}$ *and* $b \in (\Psi_{\mathrm{B}\!-\!} \cup X_B^*) \cup (\Psi_{\mathrm{C}\!-\!} \cup X_C^*)$.
...
>
> 1. *Decomposition and Module Properties: Define the disjoint sets* $\Psi_{BC\!-\!}$, $\Psi_{\mathrm{B}\!-\!}$, *and* $\Psi_{\mathrm{C}\!-\!}$ *partitioning the modules outside* $\Psi_A$. *The condition* $ch(\Psi'^{M^{c}}) = \varnothing$ *states that the set of children of the complement of* $M$ *is empty. Formally, if* $u \notin \Psi'_M$, *then* $u$ *has no children in* $G$. *This implies that all edges in* $G$ *must originate from nodes within a module.*
...
> 3. *Proof of Claim 1: Suppose* $\Psi_{BC\!-\!} \neq \varnothing$ *and let* $a \in \Psi_{BC\!-\!}$. *Since* $a \notin \Psi_A$, *we examine the path to* $X_B^*$. *The path must eventually exit* $\Psi_A$ *to reach* $X_B^*$ *or a module-specific node. In all cases, there exists an edge* $a' \to b$ *where* $a' \in \Psi_{BC\!-\!}$ *and* $b$ *lands in the specific union* $(\Psi_{\mathrm{B}\!-\!} \cup X_B^*) \cup (\Psi_{\mathrm{C}\!-\!} \cup X_C^*)$ *or leads directly to it.*
> 4. *Structural Consequence of Empty Overlap: If* $\Psi_{BC\!-\!} = \varnothing$, *then* $\Psi'_B \cap \Psi'_C \subseteq \Psi'_A$. *The property* $ch(\Psi'^{M^{c}}) = \varnothing$ *implies that for any module, there are no edges from the outside. Consequently, any node in* $\Psi'_B \setminus \Psi'_A$ *can only have parents in* $\Psi'_B$.
...
> 3. *Proof of Claim 1: We employ a proof by contradiction focused on first edge positioning. Let* $a' \to b$ *be the first edge leaving the intersection* $\Psi_{BC\!-\!}$ *along the path to* $X_B^*$. *This locks the condition* $a' \in \Psi_{BC\!-\!}$ *and* $b \notin \Psi_{BC\!-\!}$. *Since* $a' \in \Psi'_B$, *its child* $b$ *must remain in* $\Psi'_B$ *to avoid violating ancestral closure. Since* $b$ *cannot belong to* $\Psi'_S$ *or the conditioning set* $\Psi'_A$, *it must transition into the remaining valid subsets, proving* $b \in (\Psi_{\mathrm{B}\!-\!} \cup X_B^*) \cup (\Psi_{\mathrm{C}\!-\!} \cup X_C^*)$.
